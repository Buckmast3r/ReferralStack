...
export default function EditReferralModal({ referral, onClose, onUpdated }) {
  const [title, setTitle] = useState(referral.title || '');
  const [description, setDescription] = useState(referral.description || '');
  const [links, setLinks] = useState(Array.isArray(referral.links) && referral.links.length > 0 ? referral.links : [{ label: '', url: '' }]);
  const [imageUrl, setImageUrl] = useState(referral.image_url || '');
  ...
  async function handleUpdateReferral(e) {
    ...
    try {
      ...
      if (updateError) {
        ...
        toast.error('Failed to update referral card');
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-xl p-8 w-full max-w-md relative">
        ...
        <h2 className="text-2xl font-bold mb-6">Edit Referral</h2>
